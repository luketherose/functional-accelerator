  ...
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-brand-50 border border-brand-200 text-[11px] text-purple-deep font-medium">
      ...
      <button
        ...
        title={t('taxonomy.removeKeyword')}
      >
        <X size={9} />
      </button>
    </span>
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-[10px] text-text-muted shrink-0">
            {t('taxonomy.keywords', { count: cluster.keywords.length })}
          </span>
        </div>
        ...
        <div className="p-3.5 space-y-3 border-t border-surface-border bg-white">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {cluster.keywords.length === 0 && (
              <span className="text-[11px] text-text-muted italic">{t('taxonomy.noKeywords')}</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder={t('taxonomy.addKeywordPlaceholder')}
            ...
            >
              <Plus size={11} /> {t('taxonomy.addButton')}
            </button>
          </div>

          {/* Cluster name edit */}
          <div>
            <label className="text-[10px] text-text-muted font-medium block mb-1">{t('taxonomy.clusterNameLabel')}</label>
            <input
              type="text"
...
export default function TaxonomyEditor({ projectId, onClose }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setDirty(false);
      setSavedMsg(t('taxonomy.savedOk'));
      setTimeout(() => setSavedMsg(''), 3000);
    } catch {
      setSavedMsg(t('taxonomy.saveFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleRecluster = async () => {
    ...
    try {
      ...
      setReclusterMsg(t('taxonomy.reclusterOk', { count: res.runs }));
    } catch {
      setReclusterMsg(t('taxonomy.reclusterFailed'));
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden mx-4">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-border shrink-0">
          <div>
            <h2 className="text-sm font-bold text-text-primary">{t('taxonomy.title')}</h2>
        ...
        <div className="px-5 py-3.5 border-t border-surface-border bg-surface-muted/30 shrink-0 flex items-center gap-3">
          ...
          >
            ...
            {saving ? t('common.saving') : t('taxonomy.saveButton')}
          </button>
          ...
          >
            ...
            {reclustering ? t('taxonomy.reclustering') : t('taxonomy.reclusterButton')}
          </button>
