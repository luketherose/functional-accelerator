...
import {
  ...
  FileText, Scissors, Cpu, Network, GitCompare, Search, Share2, ShieldCheck, BarChart3,
} from 'lucide-react';
...
const STEPS: StepDef[] = [
  { id: 1, labelKey: 'gapAnalysis.progress.steps.parse',         descriptionKey: 'gapAnalysis.progress.steps.parseSub',         icon: FileText,    estimatedSeconds: 10,  color: 'bg-slate-500' },
  { id: 2, labelKey: 'gapAnalysis.progress.steps.chunk',         descriptionKey: 'gapAnalysis.progress.steps.chunkSub',         icon: Scissors,    estimatedSeconds: 5,   color: 'bg-blue-500' },
  { id: 3, labelKey: 'gapAnalysis.progress.steps.extract',       descriptionKey: 'gapAnalysis.progress.steps.extractSub',       icon: Cpu,         estimatedSeconds: 90,  color: 'bg-violet-500' },
  { id: 4, labelKey: 'gapAnalysis.progress.steps.relationships', descriptionKey: 'gapAnalysis.progress.steps.relationshipsSub', icon: Network,     estimatedSeconds: 45,  color: 'bg-indigo-500' },
  { id: 5, labelKey: 'gapAnalysis.progress.steps.align',         descriptionKey: 'gapAnalysis.progress.steps.alignSub',         icon: GitCompare,  estimatedSeconds: 60,  color: 'bg-amber-500' },
  { id: 6, labelKey: 'gapAnalysis.progress.steps.detect',        descriptionKey: 'gapAnalysis.progress.steps.detectSub',        icon: Search,      estimatedSeconds: 10,  color: 'bg-orange-500' },
  { id: 7, labelKey: 'gapAnalysis.progress.steps.propagate',     descriptionKey: 'gapAnalysis.progress.steps.propagateSub',     icon: Share2,      estimatedSeconds: 10,  color: 'bg-teal-500' },
  { id: 8, labelKey: 'gapAnalysis.progress.steps.verify',        descriptionKey: 'gapAnalysis.progress.steps.verifySub',        icon: ShieldCheck, estimatedSeconds: 50,  color: 'bg-purple-600' },
  { id: 9, labelKey: 'gapAnalysis.progress.steps.report',        descriptionKey: 'gapAnalysis.progress.steps.reportSub',        icon: BarChart3,   estimatedSeconds: 10,  color: 'bg-emerald-500' },
];
...
function parseStepNumber(progressStep: string | null): number {
  ...
  const m = progressStep.match(/Step (\d+)\//);
...
export default function FunctionalAnalysisProgress({ progressStep }: Props) {
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 bg-gradient-to-b from-white to-surface/40">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-[10px] text-text-muted mt-5 leading-relaxed">
          {t('gapAnalysis.progress.disclaimer')}
        </p>
      </div>
    </div>
  );
